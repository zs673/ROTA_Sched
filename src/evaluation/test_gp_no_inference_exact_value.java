...
public class test_gp_no_inference_exact_value {
...
//        EP3_Interference();                 // test Acceptance Rate U=0.6 Pr=0.3 Pe = 0-2
    ...
    public static void EP1_SchedulabilityTest() {
        ...
        for (int NoT = 4; NoT <= 8; NoT+=2) {
            ...
            for (int j = 0; j < NoS; j++) {
                ...
                result result_best = new FPS_Schedule().schedule(tasks);
                if (result_best != null) {
                    result res_0 = rwcet(result_best.jobs, overrate, overmax, hyperperiod, result_best.etss);
                    init_jobs.add(result_best.jobs);
                    if(res_0.jobs.size()==result_best.jobs.size())
                        best_schedulables++;
                }
                ...
                if (result_static != null) {
                    ...
                    if(res_0.jobs.size()==result_static.jobs.size())
                        static_schedulables++;
                }
                ...
                if (result_ri != null) {
                    result res_0 = rwcet(result_ri.jobs, overrate, overmax, hyperperiod, result_ri.etss);
                    if(res_0.jobs.size()==result_ri.jobs.size())
                        ri_schedulables++;
                }
    ...
    private static void  EP3_Interference()
    {
        ...
        for (overmax = 0.4; overmax <= 2; overmax += 0.4) {
            overrate = 0.3;
            SimpleSystemGenerator generator = new SimpleSystemGenerator(minT, maxT, LCM, NoT, NoT * 0.05, isPeriodLogUni, valueRange, seed, true);
    ...
    private static void  EP4_Interference()
    {
        ...
        for (int NoT = 4; NoT <= 14; NoT += 2) {
            ...
            while (count < 1000) {
                System.out.println("NoT: " + NoT + " times: " + count + " over rate: " + overrate + " over max: " + overmax +","  + " " + gpio_res.size()+ "," + stat_res.size() + " " + ri_res.size());
                List<List<Job>>init_jobs = new ArrayList<List<Job>>();
                List<PeriodicTask> tasks = generator.generateTasks();
                Pair<List<Job>, Long> pair = new AnalysisUtils().getJobsInHyperPeriod(tasks);
                long hyperperiod = pair.getValue();
                ...
                if (gpio_res.size() < max) {
                    ...
                    if (result_gpio != null) {
                        result res_0 = rwcet(result_gpio.jobs, overrate, overmax, hyperperiod, result_gpio.etss);
                        init_jobs.add(result_gpio.jobs);
                        Result res = new Result(result_gpio.pfs);
                        gpio_res.add(res);
                        DecimalFormat df = new DecimalFormat("#.##");
                        Double num = Double.parseDouble(df.format(res_0.jobs.size() * 1.0 / (result_gpio.jobs.size() * 1.0)));
                        gpio_num.add(num);
                ...
                count = Math.min(gpio_res.size(), Math.min(stat_res.size(),ri_res.size()));
            }
